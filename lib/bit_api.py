...
def get_proxy_ip_port():
    while 1:
        try:
            proxy_url = "http://v2.api.juliangip.com/company/postpay/getips?auto_white=1&num=1&pt=1&result_type=text&split=1&trade_no=6800520701388695&sign=50f8ec1926b647209a5aaecc307b38ed"
            response = requests.get(proxy_url)
            # print(response.text)
            ip_port = response.text.split(':')
            ip = ip_port[0]
            port = ip_port[1]
            return ip, port
        except:
...
def createBrowser(ip='', port='', username='', password=''):  # 创建或者更新窗口，指纹参数 browserFingerPrint 如没有特定需求，只需要指定下内核即可，如果需要更详细的参数，请参考文档
    if ip=='':
        proxyType = 'noproxy'
    else:
        proxyType = 'http'
    json_data = {
        ...
        'host': ip,  # 代理主机
        'port': port,  # 代理端口
        'proxyUserName': username,  # 代理账号
        'proxyPassWord': password,  # 代理账号
...
if __name__ == '__main__':
    ...
    ip = ''
    port = ''
    browser_id = createBrowser(ip, port)
